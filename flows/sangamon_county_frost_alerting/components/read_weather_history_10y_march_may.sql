...
def _normalize_payload(location: dict[str, object], payload: dict[str, object], source_year: int) -> pd.DataFrame:
...
def read_weather_history_10y_march_may(context: ComponentExecutionContext) -> pd.DataFrame:
    current_year = date.today().year
    start_year = current_year - 10
    frames: list[pd.DataFrame] = []

    for location in LOCATIONS:
        for source_year in range(start_year, current_year):
            params = {
                "latitude": location["latitude"],
                "longitude": location["longitude"],
                "start_date": f"{source_year}-03-01",
                "end_date": f"{source_year}-05-31",
                "hourly": ",".join(HOURLY_FIELDS),
                "temperature_unit": "fahrenheit",
                "timezone": SANGAMON_TIMEZONE,
            }
            payload = _request_json(params)
            frame = _normalize_payload(location, payload, source_year)
            frames.append(frame)
            log(f"Fetched {len(frame)} March-May hourly rows for {location['location_id']} in {source_year}")
