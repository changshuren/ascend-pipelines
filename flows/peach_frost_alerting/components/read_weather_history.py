...
def _normalize_frame(location: dict[str, Any], payload: dict[str, Any]) -> pd.DataFrame:
    ...
    if not isinstance(hourly, dict):
        raise ValueError("Open-Meteo archive response missing hourly object")

    units = payload.get("hourly_units")
    if not isinstance(units, dict):
        raise ValueError("Open-Meteo archive response missing hourly_units object")
...
def read_weather_history(context: ComponentExecutionContext) -> pd.DataFrame:
    ...
    for location in LOCATIONS:
        ...
        rows.append(frame)
        log(f"Fetched {len(frame)} hourly historical weather rows for {location['location_id']}")
