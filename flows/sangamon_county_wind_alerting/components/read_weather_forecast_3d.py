    ...
    for attempt in range(1, 6):
        try:
            ...
            if response.status_code in {429, 500, 502, 503, 504}:
                ...
                log(f"Transient Open-Meteo forecast error {response.status_code}; retrying in {delay_seconds} seconds")
            ...
            log(f"Forecast request attempt {attempt} failed: {exc}. Retrying in {delay_seconds} seconds")
            time.sleep(delay_seconds)
...
def read_weather_forecast_3d(context: ComponentExecutionContext) -> pd.DataFrame:
    ...
    log(f"Prepared {len(result)} total 3-day hourly forecast rows across {len(LOCATIONS)} locations")
    return result