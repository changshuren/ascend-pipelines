...
def _normalize_payload(location: dict[str, Any], payload: dict[str, Any], request_year: int) -> pd.DataFrame:
    ...
    frame = pd.DataFrame({
        "timestamp_local": pd.to_datetime(times)
    })
    ...
    frame["timestamp_utc"] = (
        frame["timestamp_local"] - pd.to_timedelta(utc_offset_seconds, unit="s")
    ).dt.tz_localize("UTC")
    ...
    frame["year"] = request_year
    frame["month"] = pd.to_datetime(frame["timestamp_utc"], utc=True).dt.month
    ...
    frame["utc_offset_seconds"] = utc_offset_seconds
...
def read_weather_history_3y_march_nov(context: ComponentExecutionContext) -> pd.DataFrame:
    current_year = date.today().year
    years = [current_year - 3, current_year - 2, current_year - 1]
    frames: list[pd.DataFrame] = []

    for location in LOCATIONS:
        for request_year in years:
            params = {
                "latitude": location["latitude"],
                "longitude": location["longitude"],
                "start_date": f"{request_year}-03-01",
                "end_date": f"{request_year}-11-30",
            ...
            frame = _normalize_payload(location, payload, request_year)
            frames.append(frame)
            log(f"Fetched {len(frame)} March-November weather rows for {location['location_id']} in {request_year}")

    result = pd.concat(frames, ignore_index=True)
    log(f"Prepared {len(result)} total retrospective hourly weather rows across {len(LOCATIONS)} locations and {len(years)} years")
    return result